...
public class AccessPerfLong {
    ...
    @Benchmark
    public void benchLongCompact(Blackhole bh) {
        if (benchCompact) {
            for (int i = 0; i < N - 7; i++) {
              bh.consume(COMPACT_ACCESS.getLong(latinBytes, i * 2 + alignOff));
            }
        } else {
            for (int i = 0; i < N - 7; i++) {
              bh.consume(UNSAFE_ACCESS.getLong(utf16Bytes, UnsafeAccess.BYTE_BASE + i * 2 + alignOff));
            }
        }
    }

    @Benchmark
    public void benchLongGeneric(Blackhole bh) {
        String str = benchCompact ? latinString : utf16String;
        for (int i = 0; i < N - 7; i++) {
          bh.consume(CHARSEQ_ACCESS.getLong(str, i * 2 + alignOff));
        }
    }
}
